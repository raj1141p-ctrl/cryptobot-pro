...
const state = {
  ...
  signalHistory: JSON.parse(localStorage.getItem("cb_signal_history") || "[]"),
...
function persistSignalHistory(){
  // Keep the most recent 2000 signal records in this browser.
  state.signalHistory=state.signalHistory.slice(0,2000);
  localStorage.setItem("cb_signal_history",JSON.stringify(state.signalHistory));
}
function recordSignal(data, action, reason){
  if(!data || !data.length) return;
  const last=data[data.length-1];
  const now=Date.now();
  const previous=state.signalHistory[0];
  // Avoid writing the same signal every 5 seconds. Record changes immediately,
  // and keep a heartbeat at most once per minute for the current signal.
  if(previous && previous.symbol===state.symbol && previous.action===action && now-previous.ts<60000) return;
  const closes=data.slice(0,-1).map(x=>x.c);
  const e9=ema(closes,9), e21=ema(closes,21), R=rsi(closes,14);
  state.signalHistory.unshift({
    ts:now, time:new Date(now).toLocaleString(), symbol:state.symbol, action,
    price:+last.c, rsi:+R.toFixed(2), ema9:+e9[e9.length-1].toFixed(4),
    ema21:+e21[e21.length-1].toFixed(4), reason:reason||""
  });
...
function pageAlgo(){
  ...
  $("algoRunOnce").onclick=()=>{const r=executeAlgoPaper(); const mm=state.markets[state.symbol]; if(mm){const sr=algoSignal(mm.candles||[]); recordSignal(mm.candles||[], sr.action, sr.reason);} toast(r.message);render()};
...
function pageJournal(){
  const tradeRows=state.trades.map(t=>`<tr><td>${t.time}</td><td>${t.symbol.replace("USDT","/USDT")}</td><td>${t.side}</td><td>${money(t.entry)}</td><td class="${t.status==="OPEN"?"":(+t.pnl>=0?"green":"red")}">${t.status==="OPEN"?"OPEN":money(t.pnl)}</td><td><span class="badge2 ${t.status==="OPEN"?"":(+t.pnl>=0?"win":"loss")}">${t.status==="OPEN"?"OPEN":(+t.pnl>=0?"WIN":"LOSS")}</span></td></tr>`).join("");
  const signalRows=state.signalHistory.map(s=>`<tr><td>${s.time}</td><td>${s.symbol.replace("USDT","/USDT")}</td><td><span class="badge2 ${s.action==="BUY"?"win":s.action==="SELL"?"loss":""}">${s.action}</span></td><td>${money(s.price)}</td><td>${Number(s.rsi).toFixed(1)}</td><td>${fmt(s.ema9)} / ${fmt(s.ema21)}</td><td>${s.reason||"--"}</td></tr>`).join("");
  $("journal").innerHTML=`<div class="hero"><div><h1>Trade Journal</h1><p>Signals and paper trades recorded automatically in this browser.</p></div><button class="btn" id="clearJournal">Clear History</button></div>
  <div class="section-title">Signal History <span class="small-muted">${state.signalHistory.length} recorded</span></div>
  <div class="card">${signalRows?`<div class="table-wrap"><table class="table"><thead><tr><th>Time</th><th>Pair</th><th>Signal</th><th>Price</th><th>RSI</th><th>EMA 9 / 21</th><th>Reason</th></tr></thead><tbody>${signalRows}</tbody></table></div>`:`<div class="empty">No signals recorded yet. Keep the live market running and CryptoBot will record them automatically.</div>`}</div>
  <div class="section-title">Paper Trade History <span class="small-muted">${state.trades.length} recorded</span></div>
  <div class="card">${tradeRows?`<div class="table-wrap"><table class="table"><thead><tr><th>Time</th><th>Pair</th><th>Side</th><th>Entry</th><th>P&L</th><th>Result</th></tr></thead><tbody>${tradeRows}</tbody></table></div>`:`<div class="empty">No paper trades yet. Use the Trade page or paper algo.</div>`}</div>`;
  $("clearJournal").onclick=()=>{if(confirm("Clear saved signals and paper-trade history?")){state.trades=[];state.positions=[];state.signalHistory=[];localStorage.removeItem("cb_trades");localStorage.removeItem("cb_positions");localStorage.removeItem("cb_signal_history");render()}};
}
...
async function refresh(){
  try{
    ...
    const currentSignal=algoSignal(state.markets[state.symbol].candles||[]);
    recordSignal(state.markets[state.symbol].candles||[], currentSignal.action, currentSignal.reason);
    render();
  }catch(e){
...
setInterval(()=>{
  if(state.algo.running){
    try{
      const mm=state.markets[state.symbol];
      if(mm) { const sr=algoSignal(mm.candles||[]); recordSignal(mm.candles||[], sr.action, sr.reason); }
      const r=executeAlgoPaper(); if(r.status!=="WAIT" && r.status!=="HOLD") { toast(r.message); render(); } }catch(e){ console.warn("Algo tick failed",e); }
  }
}, 5000);
